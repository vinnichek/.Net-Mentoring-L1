...
namespace MvcMusicStore.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public AccountController(CounterHelper<PerfomanceCounters> counterHelper, ILogger logger)
            : this(new UserManager<ApplicationUser>(new UserStore<ApplicationUser>(new ApplicationDbContext())), counterHelper, logger)
        {
        }

        public AccountController(UserManager<ApplicationUser> userManager, CounterHelper<PerfomanceCounters> counterHelper, ILogger logger)
        {
            _userManager = userManager;
            this.counterHelper = counterHelper;
            this.logger = logger;
            logger.Info("Account controller created.");
        }
        ...
        {
            logger.Info("Account controller POST: /Account/Login started.");
            ...
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    logger.Debug($"User {user.UserName} found.");

                    await SignInAsync(user, model.RememberMe);
                    counterHelper.Increment(PerfomanceCounters.SuccessLogInCounter);
            ...
            logger.Info("Account controller POST: /Account/Login finished.");
        ...
        public ActionResult LogOff()
        {
            ...
            counterHelper.Increment(PerfomanceCounters.SuccessLogOutCounter);
